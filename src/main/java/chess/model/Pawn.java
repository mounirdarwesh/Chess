...
public class Pawn extends Piece {
	 ...
	 */
	public Pawn(int postion, Color color, Board board) {
	...
	private void leftUpStep(Piece pieceAtDest, int destination) {
		...
		if((color.isWhite() && onLeftEdge()) ||
				(color.isBlack() && onRightEdge())) {
			return;
		}
	...
	private void rightUpStep(Piece pieceAtDest, int destination) {
		...
		// If the pawn is on one of the sides depending on it's color, then do nothing
		if((color.isWhite() && onRightEdge()) ||
				(color.isBlack() && onLeftEdge())) {
			return;
		}
	...
	/**
	 * Check if the pawn is on the left edge of the board
	 * @return true if it is, false otherwise
	 */
